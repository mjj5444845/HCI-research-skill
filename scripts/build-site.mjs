...
const skillDescriptionsZh = {
  ...
  "senior-researcher-core": "跨 HCI、CSCW、AI、CV、NLP 与认知科学的研究推理规范，按贡献类型匹配证据。",
  "study-design": "围绕 RQ 设计快速、可信、严谨的证据生成方案，包含方法先例、多 Agent 辩论、pilot 计划与 RQ–evidence 对齐。"
};

const agentDescriptionsZh = {
  "computational-evaluation-auditor": "审查 AI、CV、NLP 的模型、数据与评估：泄漏、基线、消融、标注歧义、计算预算与可复现性。",
...
write("exam/index.html", page({
  ...
  description: "AMSC comprehensive exam 的六主题、当前阅读队列、8–10 页写作计划、口试能力与更新状态。",
  current: "exam",
  depth: 1,
  ...
  <section class="section compact"><div class="container"><div class="exam-context" data-reveal><strong>这是一张下游视图。</strong><p>Paper、Literature、Radar、Idea 与 Study Design 工作流首先服务长期研究主线。考试页只筛选与六个考试主题、核心文献、写作论证或口试能力直接相关的部分，不接管上游研究状态。</p></div><div class="readiness-grid" data-reveal><article><strong>${sourceAvailable}/${top20.length}</strong><span>当前阅读队列 全文来源可访问</span></article><article><strong>${agentReady}/${top20.length}</strong><span>已有摘要或更深 Agent 调查</span></article><article><strong>未记录</strong><span>研究者个人阅读进度</span></article><article><strong>8–10</strong><span>目标写作页数</span></article></div><p class="readiness-warning">来源可访问、Agent 调查与研究者掌握是三个独立维度。</p>
  ...
  <section class="section"><div class="container"><div class="section-head"><div><span class="eyebrow">Priority corpus</span><h2>当前阅读队列 证据准备队列</h2></div><p>这是考试优先队列，不是长期主线的完整文献版图。已移除暂缓的机器人专属材料，保留已有阅读记录；新考试重点和队列扩展尚待确认。</p></div><div class="paper-table" role="table" aria-label="当前阅读队列 阅读准备"><div class="paper-table-head" role="row"><span>#</span><span>论文</span><span>Theme</span><span>Agent 调查</span></div>${top20.map(item => `<a class="paper-table-row" role="row" href="../papers/${escapeHtml(item.slug)}/"><span>${item.order}</span><span><strong>${escapeHtml(item.id)}</strong>${escapeHtml(item.paper.title)}</span><span>T${item.theme}</span><span>${escapeHtml(investigationLabel(item.paper.investigation_status))}</span></a>`).join("")}</div></div></section>
...
write("program/index.html", page({
  title: "AMSC 研究主线",
  description: "跨 HCI、AI、CV、NLP 的情境化多模态社会意义理解与沟通。",
  current: "program",
  depth: 1,
  body: `<section class="page-hero compact-hero"><div class="container"><span class="eyebrow">Long-term research program</span><h1>理解情境中的表达，发展具有社会意义的沟通。</h1><p>${escapeHtml(dashboard.research_line.identity)}</p><div class="button-row"><a class="button primary" href="../field-map/">查看当前证据与 Gap</a><a class="button" href="../skills/">查看服务主线的工作流</a></div></div></section>
  <section class="section compact"><div class="container"><div class="grid three"><article class="surface-card"><h2>已有研究起点</h2><p>X21 是用户作为二作参与的 EMNLP 研究。延续社会语用理解与诊断方法，明确后续新增贡献。</p><a href="../papers/x21-social-pragmatic-chinese-comments/">阅读已有研究 →</a></article><article class="surface-card"><h2>跨社区贡献</h2><p>HCI / CSCW / AI / CV / NLP。模型、方法、数据集、benchmark、理论、实证和系统均可独立贡献；人类证据按主张需要选择。</p></article><article class="surface-card"><h2>近期资源与选择</h2><p>首项研究目标3–6个月，预算有限。若干 Aria Gen 2 与一台 Quest 3 按需选用；机器人延后。具体设备能力、首项问题和方法尚未确定。</p></article></div><p>研究风格参照：Zhicong Lu（最偏好）、Judith Fan、Ziqiao Ma、Parastoo Abtahi、Ryo Suzuki。</p></div></section><section class="section compact"><div class="container"><div class="section-head"><div><span class="eyebrow">Conceptual spine</span><h2>Social Meaning · Pragmatic Understanding · Grounding · Convention · Adaptation</h2></div><p>五个节点是分析透镜，不是线性成熟阶段。选择节点可查看它在主线中的问题、证据范围与关联 Gap。</p></div><div class="program-map" data-program-map data-reveal><div class="research-chain">${programNodes.map((node, index) => `<button id="${escapeHtml(node.id)}" type="button" class="chain-node" data-program-node="${escapeHtml(node.id)}" aria-pressed="${index === 0 ? "true" : "false"}"><span>0${index + 1}</span><strong>${escapeHtml(node.label)}</strong><small>${escapeHtml(node.question)}</small></button>`).join("")}</div>${programNodes.map((node, index) => `<article class="chain-detail" data-program-detail="${escapeHtml(node.id)}" ${index === 0 ? "" : "hidden"}><div><span class="eyebrow">${escapeHtml(node.label)}</span><h2>${escapeHtml(node.question)}</h2><p>${escapeHtml(node.detail)}</p></div><dl><dt>Literature buckets</dt><dd>${escapeHtml(node.buckets.join(" · "))}</dd><dt>Related gaps</dt><dd>${node.gaps.map(gap => `<a href="../field-map/#${slugify(gap)}">${escapeHtml(gap)}</a>`).join(" · ")}</dd></dl></article>`).join("")}</div><p class="map-note">理解是近期重心，后续可研究表达、回应、共享意义与适应。图中节点不是固定论文顺序；机器人、AR和手势按问题需要加入。</p></div></section>
